...
gulp.task('compile', ['clean'], function() {
  return browserify('lib/bespoke-title.js', { standalone: 'bespoke.plugins.title' }).bundle()
    .pipe(source('bespoke-title.js'))
    .pipe(buffer())
    .pipe(header([
      '/*!',
      ' * <%= name %> v<%= version %>',
    ...
    .pipe(gulp.dest('dist'))
    .pipe(rename('bespoke-title.min.js'))
    .pipe(uglify())
    .pipe(header([
      '/*! <%= name %> v<%= version %> ',
